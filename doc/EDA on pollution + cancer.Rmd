...
Here we test whether incidence_rate has no difference between male & female groups. 

Since their incidence_rate indeed from the same distribution, we use the cancer data belongs to "male and female" category/
...
pollution_cancer <- 
  ...
  inner_join(cancer_mortality %>%
               select(year, state_abbr, AGE_ADJUSTED_RATE, mortality_rate, Mortality) %>%
               mutate(year = as.numeric(year)) %>%
...
pollution_cancer <- pollution_cancer %>%
  select(state, year, mortality_rate, aar_mortality, incidence_rate, aar_incidence,
         avg_NO3, avg_SO2, avg_TSO4, avg_TNH4, avg_CA, avg_K, avg_MG, avg_NA, avg_CL) 
